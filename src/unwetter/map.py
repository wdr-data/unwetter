...
def to_image_coords(x, y, mode):
    px = int(
        img_width - ((bbox[2] - x) * ratio_x) + abs(img_width_ - img_widths[mode]) / 2
    )
    py = (
        int((bbox[3] - y) * ratio_y + int(abs(img_height_ - img_height) / 2))
        + vertical_offset
    )
    return px, py
...
background_image = resize(
    Image.open("assets/images/bg_square.png").convert("RGBA"), Mode.SQUARE
)
studios = resize(
    Image.open("assets/images/overlay_square.png").convert("RGBA"), Mode.SQUARE
)
mask = resize(Image.open("assets/images/mask_square.png").convert("RGBA"), Mode.SQUARE)
logo_wdra = resize(
    Image.open("assets/images/logo_wdra_square.png").convert("RGBA"), Mode.SQUARE
)

legend = resize(
    Image.open("assets/images/legend_se_square.png").convert("RGBA"), Mode.SQUARE
)
legend_other = resize(
    Image.open("assets/images/legend_se_square_other.png").convert("RGBA"), Mode.SQUARE
)
...
background_image = resize(
    Image.open("assets/images/bg_wide.png").convert("RGBA"), Mode.WIDE
)
studios = resize(
    Image.open("assets/images/overlay_wide.png").convert("RGBA"), Mode.WIDE
)
mask = resize(Image.open("assets/images/mask_wide.png").convert("RGBA"), Mode.WIDE)
logo_wdra = resize(
    Image.open("assets/images/logo_wdra_wide.png").convert("RGBA"), Mode.WIDE
)

legend = resize(
    Image.open("assets/images/legend_ne_wide.png").convert("RGBA"), Mode.WIDE
)
legend_other = resize(
    Image.open("assets/images/legend_ne_wide_other.png").convert("RGBA"), Mode.WIDE
)
